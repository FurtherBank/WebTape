...
- **`_context.md`** — **整合分析上下文（推荐优先阅读）**。将时间线、请求、响应合并为单一文档，已过滤无关请求头并截断过大 body，适合快速理解全貌。
- `index.json` — 用户操作与网络请求时间线（原始数据）
- `meta.json` — 录制元数据
- `snapshots/snapshot_${id}.md` - 各操作快照数据（操作之后的页面无障碍快照）
- `requests/req_${id}.json` — 各请求的详细数据（请求头、请求体等）
- `responses/res_${id}.json` — 各响应的详细数据（状态码、响应头、响应体等）

**分析策略**：先阅读 `_context.md` 获取全貌，若需要查看被截断的完整请求/响应体，再按需查阅 `requests/` 和 `responses/` 目录下的原始文件。
