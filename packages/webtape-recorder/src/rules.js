 ...
 */
export const STATIC_EXT_RE = /\.(?:css|js|mjs|jsx|ts|tsx|png|jpe?g|gif|svg|webp|avif|ico|woff2?|ttf|eot|otf|mp4|webm|ogg|mp3|wav|map)(?:[?#]|$)/i;

/**
 * 扩展页/插件自身资源等，不参与业务链路采集。
 */
export function shouldRecordNetworkUrl(url) {
 ...
 */
export function shouldCaptureByType(resourceType, url, method) {
 ...
 * 未知类型保留（返回 true），避免误丢。
 */
export function isApiMimeType(mime) {
  if (!mime) return true;
...
export const NETWORK_IDLE_DELAY_MS = 1500;

/** 滑动窗口时长：用户操作后多久内的请求关联到该操作（毫秒） */
export const ACTION_WINDOW_MS = 2000;

/** 录制开始后忽略用户操作的宽限期（毫秒），避免误捕获启动点击 */
export const RECORDING_GRACE_MS = 800;

/**
 * 由 scheme / record-launcher 触发录制时：在已无进行中请求的前提下，
 * 持续静默超过该时间则自动停止并导出（毫秒）。
 */
export const SCHEME_AUTO_IDLE_MS = 70_000;

/**
 * scheme 录制至少持续该时长后，才允许因「空闲」自动停止（毫秒）。
 */
export const SCHEME_MIN_RECORDING_BEFORE_IDLE_MS = 55_000;

/**
 * 由 scheme / record-launcher 触发的录制上限时长，超时自动停止并导出（毫秒）。
 */
export const SCHEME_MAX_RECORDING_MS = 120_000;

/** 生成 A11y 摘要时忽略的节点角色 */
export const A11Y_IGNORED_ROLES = new Set([
  'none', 'presentation', 'generic', 'InlineTextBox', 'LineBreak',
  'ScrollArea', 'unknown',
]);

/** 仅捕获这些按键的 keydown 事件，减少噪音 */
export const SIGNIFICANT_KEYS = new Set([
