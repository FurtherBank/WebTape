...
```text
chrome-extension://jknmdokgippeemoljicabolhdnlhdgkc/record-launcher.html?url=https%3A%2F%2Fexample.com&export=webhook&webhook=http%3A%2F%2Flocalhost%3A5643%2Fwebhook
```

Popup 里的「复制外部启动页模板」生成的是同一套链接，便于核对扩展 ID 与参数格式。若省略 Webhook 相关参数，则沿用插件当前导出方式（例如下载 ZIP）。单次触发的录制时长与结束时机由扩展自动处理，无需再点停止。

👉 工作区目录、分析开关与 CLI 用法见 [WebTape Receiver](./packages/webtape-receiver/README.md)。
